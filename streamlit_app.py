...
def aplicar_bebida_desayuno(categoria, producto):
    cat, prod = filtro_estricto(categoria), filtro_estricto(producto)
    if cat == "DESAYUNO" and not any(x in prod for x in ["CALDO", "CAFE", "TE", "MATE"]):
        bebida = st.session_state.get("bebida_desayuno", "Ninguna")
        if bebida != "Ninguna":
            return f"{prod} + {filtro_estricto(bebida)}"
    return prod

# 🎨 Configuración de pantalla ultra-ancha
st.set_page_config(page_title="Restaurante Santos", layout="wide")

# CONEXIÓN SUPABASE (Solo se conecta una vez gracias a st.cache_resource)
@st.cache_resource
def init_connection():
...
for key in ['pedido_temporal', 'modo_editor', 'msj_scanner', 'ticket_imprimir', 'msj_toast']:
    if key not in st.session_state:
        st.session_state[key] = [] if key == 'pedido_temporal' else (False if key == 'modo_editor' else "")

# CACHÉ DEL MENÚ (Carga más rápido)
@st.cache_data(ttl=2)
...
def ejecutar_finalizar_venta():
    ...
    # Inserción en lote (Más rápido que de a uno)
    ...
    if conteo_comidas:
        ...
        <script>
            ...
            if (localStorage.getItem("ultimo_ticket") !== ticketId) {{
                ...
                var printIframe = parentDoc.getElementById('print-iframe') || parentDoc.createElement('iframe');
                printIframe.id = 'print-iframe';
                printIframe.style.position = 'absolute';
                printIframe.style.top = '-10000px';
                if(!parentDoc.getElementById('print-iframe')) parentDoc.body.appendChild(printIframe);
                
                var iframeDoc = printIframe.contentWindow.document;
                iframeDoc.open();
                iframeDoc.write("<html><head><meta charset='UTF-8'><style>@page {{ margin: 0; }} body {{ margin: 0; padding: 0; font-family: Arial, sans-serif; color: black; }}</style></head><body>" + document.getElementById("ticket-content").innerHTML + "</body></html>");
                iframeDoc.close();
                setTimeout(function() {{ printIframe.contentWindow.focus(); printIframe.contentWindow.print(); }}, 1000); 
            }}
        </script>
        """
        st.session_state.msj_toast = "✅ Venta con comida (Ticket en cocina)"
    else:
        st.session_state.ticket_imprimir = ""
        st.session_state.msj_toast = "✅ Venta rápida registrada."
...
def modal_nuevo(cat):
    ...
    if st.button("Guardar") and n and p:
...
def modal_otros():
    desc, m = st.text_input("¿Qué se vendió?"), st.number_input("Monto ($)", min_value=0, step=100, value=None)
    if st.button("Agregar al Pedido") and desc and m:
        st.session_state.pedido_temporal.append({"categoria": "OTROS", "producto": filtro_estricto(desc), "monto": int(m)})
        st.rerun()

@st.dialog("💸 Gasto")
def modal_gastos():
    m, d = st.number_input("Monto ($)", min_value=0, step=500, value=None), st.text_input("Descripción")
    if st.button("Guardar Gasto") and m:
...
def modal_cajavecina():
    ...
    if st.button("✅ Agregar", type="primary", use_container_width=True) and m_custom:
...
def modal_venta_debo(total_venta):
    ...
    if st.button("✅ Confirmar Venta", type="primary", use_container_width=True):
        if nom and deuda > 0:
            ...
            ejecutar_finalizar_venta(); st.rerun()
...
def procesar_scanner():
    ...
    if codigo and "codigo" in df_menu.columns:
        ...
        if not encontrado.empty:
            ...
            prod = aplicar_bebida_desayuno(row['categoria'], row['producto'])
            st.session_state.pedido_temporal.append({"categoria": filtro_estricto(row['categoria']), "producto": prod, "monto": row['monto']})
            st.session_state.msj_toast = f"✅ {prod} agregado"
        else:
...
with col_m:
    def mostrar_seccion(titulo, cat, especial=None):
        ...
        if especial == "Otros":
            ...
            items = df_menu[df_menu["categoria"].apply(lambda x: filtro_estricto(str(x))) == filtro_estricto(cat)]
            for i, (_, row) in enumerate(items.iterrows()):
                with grid[i % 5]:
                    if st.session_state.modo_editor:
                        if st.button("❌", key=f"d_{row['id']}"):
                            supabase.table("menu_dia").delete().eq("id", row['id']).execute(); st.cache_data.clear(); st.rerun()
                    if st.button(f"{row['producto']}\n\n${int(row['monto']):,}".replace(",", "."), key=f"b_{row['id']}", use_container_width=True):
                        st.session_state.pedido_temporal.append({
                            ...
                            "producto": aplicar_bebida_desayuno(row['categoria'], row['producto']), 
                            "monto": row['monto']
                        })
                        st.rerun()
            if st.session_state.modo_editor:
    ...
    with st.expander("🍔 COMIDA", expanded=True):
        ...
        with t1: 
            st.radio("☕ Bebida incluida:", ["Ninguna", "Té", "Café", "Mate"], horizontal=True, key="bebida_desayuno")
...
with col_p:
    ...
    for i, item in enumerate(st.session_state.pedido_temporal):
        ...
        if cbt.button("🗑️", key=f"del_{i}"): st.session_state.pedido_temporal.pop(i); st.rerun()
