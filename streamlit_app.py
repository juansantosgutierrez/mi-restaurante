...
# --- FUNCIONES DE BASE DE DATOS (LECTURA REAL) ---
def leer_menu_desde_excel():
    try:
        # ttl=0 obliga a la app a leer el Excel fresco cada vez
...
def modal_nuevo(cat):
    ...
    if st.button("Confirmar y Guardar"):
        if n and p > 0:
            df_act = leer_menu_desde_excel()
            df_n = pd.DataFrame([{"Categoria": cat, "Producto": n, "Monto": p}])
            df_f = pd.concat([df_act, df_n], ignore_index=True)
            conn.update(spreadsheet=URL_DIRECTA, worksheet="Menu_Dia", data=df_f)
            st.rerun()
...
def modal_gastos():
    ...
    if st.button("Guardar Gasto"):
        df_g = pd.DataFrame([{"Fecha": datetime.now().strftime("%Y-%m-%d %H:%M"), "Monto": m, "Descripcion": d}])
...
# --- INTERFAZ PRINCIPAL ---
df_menu = leer_menu_desde_excel()

c1, c2, c3 = st.columns([2, 1, 1])
c1.title("🍴 Restaurante Santos")
if c2.button("💸 GASTOS", use_container_width=True): modal_gastos()
btn_txt = "🔄 CERRAR EDITOR" if st.session_state.modo_editor else "➕ AGREGAR MENU HOY"
if c3.button(btn_txt, use_container_width=True):
...
with col_m:
    def mostrar_seccion(titulo_pantalla, nombre_cat):
